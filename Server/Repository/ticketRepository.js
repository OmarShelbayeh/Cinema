...
history = async (id) => {
  ...
  History = await db.sequelize.query(
    "select m.name as name,  count(t.seat_id) as tickets, s.showing_at as date from tickets t INNER JOIN schedules s on t.schedule_id = s.id INNER JOIN movies m on s.movie_id = m.id WHERE who_id = :id and s.showing_at < DATE(NOW()) GROUP BY m.name, s.showing_at;",
    {
      replacements: { id: id },
...
showTickets = async (id) => {
  ...
  Tickets = await db.sequelize.query(
    "select t.id,  m.name as name, t.seat_id as seat, s.showing_at as date from tickets t INNER JOIN schedules s on t.schedule_id = s.id INNER JOIN movies m on s.movie_id = m.id WHERE who_id = :id ;",
    {
      replacements: { id: id },
      type: db.sequelize.QueryTypes.SELECT,
    }
